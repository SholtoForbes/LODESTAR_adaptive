...
num2words(-1024)
%% Decimal Digits
% |NUM2WORDS| rounds the number value to the digit specified by the
% |order| or |sigfig| options: |sigfig| rounds the input value to
% the requested number of significant figures, while |order| rounds to
% the specified order. The default |order| is zero, which means that
% all fractional digits are rounded to give an integer. Decimal digits
% can therefore be returned by selecting an appropriate value for either
% the |order| or |sigfig| options:
num2words(1.234) % default 'order' is zero
num2words(1.234, 'order',-2)
num2words(1.234, 'sigfig',3)
%% Trailing Zeros
% If the trailing decimal digits are zeros, then by default they are not
% returned in the string. Setting the |trz| option to |true| will keep
...
%% Type: Highest
% This uses the highest multiplier/scale (i.e. only one multiplier word)
% together with a significand that includes decimal digits when required. In
% most cases it will be useful to also specify the number of significant figures:
...
% |WORDS2NUM|> converts a string (with a number given in English words)
